...
.tpid-enterprise p{margin:0 0 12px;color:rgba(255,255,255,.48)}
.tpid-enterprise a{color:rgba(255,255,255,.78);text-decoration:underline;text-underline-offset:3px}
.tpid-brand-name{color:#fff!important}
.tpid-light .tpid-enterprise{color:rgba(0,0,0,.62)}
...
<div class="tpid-backdrop" id="tpid-backdrop">
  <div class="tpid-modal" role="dialog" aria-modal="true" aria-label="TOKEN PAY ID">
    ...
    <section class="tpid-enterprise" id="tpid-enterprise" aria-label="Приложение">
      <img class="tpid-enterprise-mark" id="tpid-enterprise-icon" src="${ORIGIN}/tokenpay-icon.png" alt="" hidden>
      <h2 id="tpid-enterprise-name">TOKEN PAY ID</h2>
      <p id="tpid-enterprise-description">Единая система безопасной аутентификации.</p>
      <a id="tpid-enterprise-url" href="${ORIGIN}" target="_blank" rel="noopener noreferrer">tokenpay.space</a>
    </section>
  ...
  function loadEnterpriseBranding(clientId) {
    if (!clientId) return;
    fetch(API + '/oauth/branding?' + new URLSearchParams({ client_id: clientId }), { headers: { Accept: 'application/json' } })
      .then(function(r) { if (!r.ok) throw new Error('branding unavailable'); return r.json(); })
      .then(function(data) {
        var app = data && data.app;
        if (!app) return;
        var name = $id('tpid-enterprise-name');
        var description = $id('tpid-enterprise-description');
        var icon = $id('tpid-enterprise-icon');
        var link = $id('tpid-enterprise-url');
        if (name && app.app_name) name.textContent = app.app_name;
        if (description && app.description) description.textContent = app.description;
        if (icon && app.icon_url) {
          try { var iconUrl = new URL(app.icon_url); if (iconUrl.protocol === 'https:') { icon.src = iconUrl.href; icon.hidden = false; } } catch (_) {}
        }
        if (link && app.client_uri) {
          try { var siteUrl = new URL(app.client_uri); if (siteUrl.protocol === 'https:') { link.href = siteUrl.href; link.textContent = siteUrl.hostname; } } catch (_) {}
        }
      }).catch(function() {});
  }
  ...
  var TPID = {
    ...
    init: function(cfg) {
      ...
      injectStyles();
      injectModal();
      loadEnterpriseBranding(cfg && cfg.clientId);
