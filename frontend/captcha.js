...
(function(global) {
    ...
    function _showModal(onSuccess, onFail) {
        _fetchChallenge().then(ch => {
            let { nonce, hole_x, hole_y, width, height, piece_size } = ch;
            ...
            style.textContent = `
                ...
                .tpc-error{font-size:.72rem;color:rgba(255,100,100,.8);text-align:center;margin-bottom:10px;display:none}
                ...
                .tpc-success-icon{width:48px;height:48px;border-radius:50%;background:rgba(80,220,120,.1);border:2px solid rgba(80,220,120,.3);display:flex;align-items:center;justify-content:center;margin:0 auto 12px}
            `;
            ...
            function onEnd() {
                ...
                _verifySolution(nonce, canvasX).then(d => {
                    if (d && d.success) {
                        ...
                        thumb.style.background = 'rgba(80,220,120,.9)';
                        fill.style.background = 'rgba(80,220,120,.15)';
                        setTimeout(() => {
                            _cleanup();
                            overlay.remove();
                            style.remove();
    ...
    function check() {
        return new Promise((resolve) => {
            ...
            _showModal(resolve, () => resolve(null));
        });
    }
    ...
    global.TokenPayCaptcha = { check, show, reset };
