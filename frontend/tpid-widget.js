...
.tpid-err{display:none;background:rgba(255,80,80,.07);border:1px solid rgba(255,80,80,.18);border-radius:8px;padding:9px 12px;font-size:.78rem;color:rgba(255,160,160,.9);margin-top:10px;text-align:center}
...
.tpid-light .tpid-err{background:rgba(239,68,68,.05);border-color:rgba(239,68,68,.15);color:rgba(220,38,38,.85)}
...
<div class="tpid-backdrop" id="tpid-backdrop">
  ...
  function goStep2() {
    ...
    if (!email || !email.includes('@')) {
      ...
      if (el) { el.style.borderColor = 'rgba(255,100,100,.4)'; setTimeout(function() { el.style.borderColor = ''; }, 1500); }
      return;
    }
